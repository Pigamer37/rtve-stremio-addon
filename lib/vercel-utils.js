...
exports.GetVercelBlob = async function (fileName, type = 'json') {
  return vercelBlob.get(fileName, { access: 'public' }).then(res => {
    if (res?.blob?.uploadedAt) {
      const timeDiff = (new Date() - res.blob.uploadedAt) / 86400000 // 1000*60*60 ms->s->min->h
      if (timeDiff >= 20) throw Error('File is stale and should be renewed')
    }
  ...
  .then(blobUrl => {
    return fetch(blobUrl).then((resp) => {
      ...
      return (type === 'json') ? resp.json() : resp.arrayBuffer()
    })
  })
}
